...
function iniciarPorNombre(nombre) {
  ...
  t.timerID = setInterval(() => {
    ...
    const tiempoTotal = (Number(t.tiempoAcumulado) || 0) + ((t.inicio instanceof Date && !isNaN(t.inicio.getTime())) ? (ahora - t.inicio)/1000 : 0);
...
function guardarEstado() {
  try {
    const actividadesEnOrden = Array.from(document.querySelectorAll("#tabla tbody tr")).map(fila => {
      const nombre = fila.children[1].innerText;
      return sanitizeActividad(tiempos[nombre] || { nombre });
    });
    localStorage.setItem("estadoSMED", JSON.stringify({ actividades: actividadesEnOrden }));
...
function cargarEstado() {
  try {
    const estadoGuardado = localStorage.getItem("estadoSMED");
    if (!estadoGuardado) return;
        const parsed = JSON.parse(estadoGuardado);
    const actividades = parsed && parsed.actividades ? parsed.actividades : null;
    if (!actividades) return;
        actividades.forEach(raw => {
      const safe = {
        nombre: raw.nombre,
        inicio: raw.inicio && !isNaN(new Date(raw.inicio).getTime()) ? new Date(raw.inicio) : null,
        fin: raw.fin && !isNaN(new Date(raw.fin).getTime()) ? new Date(raw.fin) : null,
        tiempoAcumulado: Number(raw.tiempoAcumulado) || 0,
        duracion: Number(raw.duracion) || 0,
        estado: raw.estado || 'detenido',
        responsable: raw.responsable || '',
        timerID: null
      };
      tiempos[safe.nombre] = safe;
      const celdaDuracion = document.getElementById(`duracion-${safe.nombre.replace(/\s+/g, "_")}`);
      if (safe.estado === 'corriendo' && safe.inicio) {
        iniciarTimerSeguro(safe, celdaDuracion);
      } else {
        if (celdaDuracion) celdaDuracion.innerText = formatearTiempo(safe.tiempoAcumulado);
      }
    });
      ...
      if (p.estado === "corriendo") {
        ...
        if (p.inicio && !isNaN(new Date(p.inicio).getTime())) {
          ...
          p.timerID = setInterval(() => {
            const ahora = new Date();
            const tiempoTotal = (Number(p.tiempoAcumulado) || 0) + ((p.inicio instanceof Date && !isNaN(p.inicio.getTime())) ? (ahora - p.inicio)/1000 : 0);
            celda.innerText = formatearTiempo(tiempoTotal);
          }, 100);
        } else {
          // Si la fecha no es válida, poner en estado pausado
...
// --- SAFETY HELPERS ADDED BY ASSISTANT ---
function sanitizeActividad(a) {
  return {
    nombre: a.nombre || '',
    inicio: a.inicio ? new Date(a.inicio).toISOString() : null,
    fin: a.fin ? new Date(a.fin).toISOString() : null,
    tiempoAcumulado: Number(a.tiempoAcumulado) || 0,
    duracion: Number(a.duracion) || 0,
    estado: a.estado || 'detenido',
    responsable: a.responsable || ''
  };
}

function iniciarTimerSeguro(t, celda) {
  if (!celda) return;
  if (t.timerID) clearInterval(t.timerID);
  t.timerID = setInterval(() => {
    const ahora = new Date();
    const acum = Number(t.tiempoAcumulado) || 0;
    const inicioValido = t.inicio instanceof Date && !isNaN(t.inicio.getTime());
    const delta = inicioValido ? (ahora - t.inicio) / 1000 : 0;
    let tiempoTotal = acum + delta;
    if (!isFinite(tiempoTotal) || isNaN(tiempoTotal)) {
      console.warn('Tiempo inválido para', t.nombre, { acum, inicio: t.inicio });
      tiempoTotal = acum;
    }
    celda.innerText = formatearTiempo(tiempoTotal);
  }, 100);
